...
_convert_loong64() {
	...
	sed -e 's|^Architecture: loongarch64$|Architecture: mips64el|g' \
	    -i "$DEBDIR"/metadata/control
        if grep -q "Depends:" "$DEBDIR"/metadata/control; then
	        sed -i 's/Depends:/Depends: latx-loongarch, binfmt-support,/g' "$DEBDIR"/metadata/control
        else
	        echo "Depends: latx-loongarch, binfmt-support" >> "$DEBDIR"/metadata/control
        fi
